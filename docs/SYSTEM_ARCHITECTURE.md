...
### 算法描述
针对钢铁行业的RAG检索系统，集成**钢铁专业术语增强**、**知识图谱关联查询**和**工艺参数语义理解**，实现对钢种(Q235/304不锈钢)、工艺流程(热轧/退火)、设备规范等领域知识的精准检索。
...
1:  // 钢铁专业术语增强查询
2:  enhanced_query ← SteelVocabularyExpander(query)  // Q235 → Q235碳素结构钢
3:  IF enhanced_query.has_steel_grade THEN  // 检测钢种代号
4:      kg_entities ← KnowledgeGraph.query_steel_grade(enhanced_query.steel_grade)
5:      enhanced_query ← enhanced_query + kg_entities.properties  // 加入抗拉强度、延伸率等
6:  END IF
7:  
8:  // 向量化 (针对钢铁领域fine-tuned模型)
9:  query_vector ← SteelEmbedder.embed([enhanced_query])  // 384维向量
10: query_vector ← L2_normalize(query_vector)
11: 
12: // 混合检索: 向量检索 + 钢铁知识图谱过滤
13: vector_results ← FAISS.search(query_vector, k=top_k×2)  // O(384·n)
14: FOR result IN vector_results DO
15:     // 知识图谱增强评分
16:     kg_relevance ← 0
17:     IF result.metadata.contains_steel_grade THEN
18:         related_processes ← KnowledgeGraph.get_relations(
19:             entity=result.steel_grade, 
20:             relation_type="USES_PROCESS"  // 钢种→工艺关系
21:         )
22:         IF agent_type == "PROCESS" AND related_processes NOT empty THEN
23:             kg_relevance ← 0.3  // 工艺Agent权重提升
24:         END IF
25:     END IF
26:     result.final_score ← result.vector_score × 0.7 + kg_relevance × 0.3
27: END FOR
28: 
29: // 按钢铁领域相关性重排序
30: ranked_results ← ReRank(vector_results, by="final_score", top_k=top_k)
31: RETURN ranked_results
```
...
$$
R@k_{\text{enhanced}} \geq R@k_{\text{raw}} + \Delta_{\text{kg}}其中 $\Delta_{\text{kg}} \in [0.15, 0.35]$（实验测量，基于钢铁测试集）
$$
**定理 2.2** (混合检索最优权重):  
...
### 算法描述
基于ReAct框架的**角色专业化智能体系统**，根据用户角色(PRODUCTION/TECHNICIAN/PURCHASER)自动切换Agent类型，调用钢铁领域专属工具(工艺模拟、设备诊断、市场分析)。
...
1:  // 角色到Agent映射 (钢铁领域专业化)
2:  agent_map ← {
3:      "PRODUCTION": ProcessAgent,      // 工艺优化、质量控制
4:      "TECHNICIAN": EquipmentAgent,    // 设备故障诊断
5:      "PURCHASER": MarketAgent,        // 原料价格分析
6:      "ENV_EXPERT": EnvironmentAgent   // 环保标准检索
7:  }
8:  active_agent ← agent_map[user_role]
9:  
10: // 钢铁领域意图识别
11: intent ← ClassifyIntent(query, categories=[
12:     "工艺参数咨询",    // 如 "热轧温度范围"
13:     "设备故障诊断",    // 如 "轧机振动异常"
14:     "钢种性能查询",    // 如 "Q345B抗拉强度"
15:     "标准规范查询"     // 如 "GB/T 700-2006"
16: ])
17: 
18: // 记忆更新 (包含钢铁专业术语上下文)
19: Memory.add_user(query)
20: recent_history ← Memory.as_messages(max_turns=10)
21: 
22: // 构建角色专属Prompt (钢铁领域)
23: system_prompt ← BuildSteelExpertPrompt(
24:     role=user_role,
25:     specialization=active_agent.domain,  // 如 "钢铁热轧工艺专家"
26:     steel_context=ExtractSteelEntities(query)  // 提取 Q235、热轧等实体
27: )
28: 
29: // ReAct推理 + 钢铁领域工具调用
30: reasoning_steps, tool_calls ← [], []
31: FOR step IN active_agent.iterate(query, recent_history, system_prompt) DO
32:     IF step.type == "TOOL_CALL" THEN
33:         // 钢铁专属工具
34:         IF step.tool_name == "SteelGradeQuery" THEN
35:             result ← KnowledgeGraph.query_steel_properties(step.args.steel_grade)
36:         ELSE IF step.tool_name == "ProcessSimulator" THEN
37:             result ← SimulateProcess(step.args.params)  // 工艺参数仿真
38:         ELSE IF step.tool_name == "EquipmentFaultDiagnoser" THEN
39:             result ← DiagnoseEquipment(step.args.symptoms)  // 故障诊断
40:         END IF
41:         reasoning_steps.append({action: step.tool_name, observation: result})
42:         tool_calls.append({tool: step.tool_name, args: step.args})
43:     ELSE IF step.type == "FINAL_ANSWER" THEN
44:         final_answer ← step.content
45:         BREAK
46:     END IF
47: END FOR
48: 
49: // 记忆持久化
50: Memory.add_assistant(final_answer, metadata={
51:     "agent_type": active_agent.type,
52:     "steel_entities": ExtractSteelEntities(query),
53:     "tool_calls": tool_calls
54: })
55: RETURN {answer: final_answer, reasoning_steps, tool_calls}
```
...
### 算法描述
针对钢铁技术文档的**专业实体识别**、**工艺参数提取**和**智能分块**，支持PDF钢种手册、工艺规程、设备说明书等多格式文献的结构化处理。
...
1:  // 多格式加载 (针对钢铁技术文档)
2:  ext ← GetFileExtension(file_path)
3:  IF ext == ".pdf" THEN
4:      raw_text ← LoadPDFWithPyMuPDF(file_path)  // 保留表格、公式
5:      raw_text ← FixPDFArtifacts(raw_text)      // 修复"Q 2 3 5"→"Q235"
6:  ELSE IF ext IN {".docx", ".doc"} THEN
7:      raw_text ← LoadWord(file_path)
8:  END IF
9:  
10: // 钢铁专业文本清洗
11: clean_text ← RemoveNonSteelChars(raw_text)   // 保留钢种代号特殊字符
12: clean_text ← NormalizeParameters(clean_text)  // 统一 "σb=450MPa" 格式
13: 
14: // 钢铁领域实体识别 (NER)
15: steel_entities ← []
16: FOR entity_type IN [STEEL_GRADE, ALLOY_ELEMENT, PROCESS, EQUIPMENT, STANDARD] DO
17:     IF entity_type == STEEL_GRADE THEN
18:         entities ← RegexExtract(clean_text, patterns=[
19:             r'Q\d{3}[A-Z]?',      // Q235, Q345B
20:             r'\d+#',              // 20#, 45#钢
21:             r'SUS\d{3}[A-Z]?'     // SUS304不锈钢
22:         ])
23:     ELSE IF entity_type == ALLOY_ELEMENT THEN
24:         entities ← DictMatch(clean_text, vocabulary=ALLOY_ELEMENTS)
25:         // 提取 "C: 0.17-0.24%" → {element: "C", range: [0.17, 0.24]}
26:     ELSE IF entity_type == PROCESS THEN
27:         entities ← PatternMatch(clean_text, patterns=[
28:             r'热轧温度[:：]\s*(\d+)[-~](\d+)\s*℃',  // 工艺参数
29:             r'退火时间[:：]\s*(\d+)\s*小时'
30:         ])
31:     END IF
32:     steel_entities.extend(entities)
33: END FOR
34: 
35: // 基于钢铁语义的智能分块
36: chunks ← []
37: buffer, current_size ← [], 0
38: FOR sentence IN SplitByChinesePeriod(clean_text) DO
39:     // 检测钢铁章节边界 (如 "3.2 热轧工艺参数")
40:     IF IsProcessSectionHeader(sentence) THEN
41:         IF buffer NOT empty THEN
42:             chunks.append(CreateSteelChunk(buffer, steel_entities))
43:             buffer, current_size ← [], 0
44:         END IF
45:     END IF
46:     
47:     IF current_size + len(sentence) > chunk_size AND buffer NOT empty THEN
48:         chunk_text ← JOIN(buffer)
49:         chunk_hash ← MD5(chunk_text)
50:         // 为每个chunk关联识别的钢铁实体
51:         chunk_steel_entities ← FilterEntitiesInRange(
52:             steel_entities, 
53:             start=buffer[0].pos, 
54:             end=buffer[-1].pos
55:         )
56:         metadata ← {
57:             file: basename(file_path),
58:             chunk_id: len(chunks),
59:             hash: chunk_hash,
60:             steel_entities: chunk_steel_entities,  // 关键增强
61:             entity_types: [e.type for e in chunk_steel_entities],
62:             has_parameters: ContainsProcessParams(chunk_text)
63:         }
64:         chunks.append({text: chunk_text, metadata: metadata})
65:         buffer, current_size ← [], 0
66:     END IF
67:     buffer.append(sentence)
68:     current_size ← current_size + len(sentence)
69: END FOR
70: 
71: IF buffer NOT empty THEN
72:     chunks.append(CreateSteelChunk(buffer, steel_entities))
73: END IF
74: RETURN chunks
```
