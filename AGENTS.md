...
1. **多工艺流程选择** ✨: 支持 6 种标准工艺路线切换
   ...
   - 真空/特殊精炼路线（高端钢种）
   - 连铸—热轧一体线（CC–HSM）
2. **交互式工艺流程图**: 钢铁生产全流程可视化（原料→炼铁→炼钢→轧钢→成品）
3. **工艺节点详情**: 查看每个工序的标准参数、设备信息、能耗数据
4. **节点增删改功能** 🆕: 自由编辑工艺流程
   - ➕ 添加自定义节点（智能插入位置选择）
   - 📍 插入位置：在开头/末尾/指定节点之前/之后
   - 🔗 自动建立节点连接关系
   - ✏️ 编辑节点信息和参数
   - 🗑️ 删除不需要的节点
   - 📝 可视化参数编辑对话框
   - 🎨 **自动布局优化** ✨：插入/删除节点后自动刷新布局，保持美观整洁
5. **多视图切换**: 支持流程图视图和列表视图
6. **参数监控**: 展示标准参数值、温度范围、能耗、物料损失率等
7. **碳排放信息** ✨: 显示每种工艺的吨钢碳排放范围（t CO₂/t 钢）
8. **缩放控制**: 支持流程图缩放（50%-150%）
...
#### 2. 选择工艺流程
- **工艺流程选择器**: 点击页面右上角的工艺流程下拉菜单
- **可用工艺路线**:
  1. **高炉—转炉（BF→BOF）长流程** - 传统路线，适合大规模连续生产（碳排放：1.8-2.5 t CO₂/t）
  2. **废钢电炉（EAF）短流程** - 低碳路线，适合弹性生产（碳排放：0.3-0.9 t CO₂/t）
  3. **直接还原（DRI）→ 电炉** - 气基还原，适合低碳场景（碳排放：0.4-1.0 t CO₂/t）
  4. **COREX / FINEX 熔融还原** - 减少焦炭依赖（碳排放：1.2-2.0 t CO₂/t）
  5. **真空/特殊精炼路线** - 高端合金钢、不锈钢（碳排放：0.5-2.5 t CO₂/t）
  6. **连铸—热轧一体线（CC–HSM）** - 高效率板带产线（碳排放：0.4-1.2 t CO₂/t）

#### 3. 查看工艺流程
- **流程图视图**: 点击页面左上角的"流程视图"查看完整生产流程
- **列表视图**: 切换到"列表视图"查看所有工艺节点列表
- **缩放控制**: 使用 + / - 按钮调整流程图缩放比例（50%-150%）
- **工艺描述**: 页面头部显示当前工艺的详细描述和适用场景

#### 4. 查看节点详情
- 点击流程图中的任意节点（如"转炉炼钢"或"电弧炉熔炼"）
- 右侧面板显示节点详细信息：
  - **工艺参数**: 温度范围、停留时间、关键参数
  - **能耗数据**: 电耗、燃料耗、气耗等
  - **物料损失**: 物料损失率百分比
...
```bash
# 1. 点击页面右上角"编辑模式"按钮
# 2. 系统会克隆当前工艺流程数据到编辑状态
# 3. 界面显示"编辑中"标签和编辑控制按钮
```
...
- 填写节点信息（名称、类型、描述）
- **选择插入位置**：
  - 🎯 **在流程开头**：新节点将成为第一个节点，自动连接到原第一个节点
  - 🎯 **在流程末尾**：新节点将成为最后一个节点，自动连接到原最后节点
...
- 方法2：选择节点 → 右侧详情面板 → 点击"编辑节点"按钮
- 修改节点信息和工艺参数
- 点击"保存修改"

**删除节点**：
- 方法1：列表视图 → 点击节点右侧的删除按钮 🗑️
- 方法2：选择节点 → 右侧详情面板 → 点击"删除节点"按钮
- 确认删除（删除会同时移除相关连线）
...
| 工艺路线 | 节点数 | 碳排放 (t CO₂/t) | 适用场景 | 优势 | 劣势 |
|---------|-------|-----------------|---------|------|------|
| **BF-BOF** | 8 | 1.8-2.5 | 大型综合钢厂 | 产能大、成熟稳定 | 碳排放高、焦炭依赖 |
| **EAF** | 4 | 0.3-0.9 | 中小型钢厂、废钢丰富区域 | 低碳、弹性生产 | 电价敏感、废钢质量依赖 |
| **DRI-EAF** | 4 | 0.4-1.0 | 天然气/氢气资源充足 | 低碳、质量可控 | 气源依赖、投资大 |
| **COREX/FINEX** | 3 | 1.2-2.0 | 减少焦炭依赖 | 副产煤气回收 | 技术复杂 |
| **真空精炼** | 3 | 0.5-2.5 | 高端特种钢 | 超净钢、精度高 | 产能小、成本高 |
| **CC-HSM** | 3 | 0.4-1.2 | 板带材产线 | 高效率、高表面质量 | 对连铸质量要求高 |

### 工艺流程节点说明

#### 高炉—转炉（BF-BOF）长流程主要工序

| 节点名称 | 类型 | 说明 | 关键参数 |
|---------|------|------|---------|
| **原料准备** | 物料 | 铁矿石、焦炭、石灰石储存和配料 | 铁品位(50-65%)、粒度(0-50mm) |
| **炼焦** | 工艺流程 | 炼焦煤干馏制焦炭 | 温度(900-1100℃)、焦炭收率(60-70%) |
| **烧结** | 工艺流程 | 铁矿粉烧结成块矿 | 温度(1200-1400℃)、Fe含量(55-60%) |
| **高炉炼铁** | 工艺流程 | 还原冶炼成铁水 | 温度(1500-2000℃)、焦比(300 kg/t铁) |
| **转炉炼钢** | 工艺流程 | 铁水吹氧脱碳炼钢 | 温度(1550-1700℃)、终碳(0.02-0.2%) |
| **炉外精炼** | 工艺流程 | 钢水脱硫脱氧、合金化 | 温度(1550-1650℃)、硫(<50ppm) |
| **连续铸造** | 工艺流程 | 钢水连续浇铸成板坯 | 温度(1400-1600℃)、铸速(0.6-1.5 m/min) |
| **轧制与热处理** | 工艺流程 | 板坯加热→轧制→退火 | 加热温度(1100-1250℃)、终轧温度(850-950℃) |

#### 废钢电炉（EAF）短流程主要工序

| 节点名称 | 类型 | 说明 | 关键参数 |
|---------|------|------|---------|
| **废钢回收与准备** | 物料 | 废钢剪切、配级、除杂 | 粒度(≤300mm)、准备时间(30-120分钟) |
| **电弧炉熔炼** | 工艺流程 | 废钢电弧熔化+吹氧脱碳 | 温度(1550-1650℃)、电耗(400-600 kWh/t) |
| **炉外精炼** | 工艺流程 | 钢水脱硫脱氧、合金化 | 温度(1500-1650℃)、硫(<50ppm)、氧(<20ppm) |
| **连铸→轧制** | 工艺流程 | 精钢水→板坯→热/冷轧 | 铸造温度(1400-1560℃)、铸速(0.6-1.2 m/min) |
...
│   ├── constants/
│   │   ├── processData.ts              # 原有工艺流程数据（已废弃）
│   │   └── steelProcessTemplates.ts    # ✨ 新增：多工艺流程模板数据
```

**新增数据结构** (`steelProcessTemplates.ts`):
```typescript
// 工艺流程模板接口
...
export const PROCESS_TEMPLATES: ProcessTemplate[] = [
    BF_BOF_TEMPLATE,        // 高炉—转炉长流程
    EAF_TEMPLATE,           // 废钢电炉短流程
    DRI_EAF_TEMPLATE,       // 直接还原→电炉
    COREX_TEMPLATE,         // COREX/FINEX熔融还原
    VACUUM_SPECIAL_TEMPLATE,// 真空/特殊精炼路线
    CC_HSM_TEMPLATE,        // 连铸—热轧一体线
];
```
...
6. ✅ 参数详情展示（标准值、单位、范围）
7. ✅ **节点增删改功能** 🆕：编辑模式、添加节点、编辑节点、删除节点
8. ✅ **智能节点插入** ✨：选择插入位置（开头/末尾/之前/之后），自动计算坐标和连线
9. ✅ **自动布局优化** 🎨：插入/删除节点后自动刷新布局，层次分明、间距均匀
10. ✅ **手动刷新布局** 🔄：一键优化流程图布局，保持美观整洁
...
1. ✅ **选择合适的工艺流程** ✨：
   ...
   - 高端特种钢：选择 **真空/特殊精炼路线**
   - 板带材产线：选择 **CC-HSM 一体线**
2. ✅ **对比不同工艺**：切换工艺流程，对比节点数量、碳排放数据
3. ✅ **先查看流程图**：了解完整生产流程
4. ✅ **点击节点查看详情**：深入了解每个工序的参数、能耗数据
5. ✅ **使用缩放功能**：调整流程图大小以获得最佳视野
6. ✅ **切换视图模式**：流程图适合整体浏览，列表视图适合快速定位
7. ✅ **关注碳排放信息** ✨：页面右上角显示当前工艺的吨钢碳排放范围
8. ✅ **关注节点状态**：注意异常节点（红色边框）的参数
9. ❌ 避免频繁切换视图（影响性能）
