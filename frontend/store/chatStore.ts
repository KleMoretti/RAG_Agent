...
interface ChatState {
    ...
    currentUserId: number | null; // 当前登录用户的 ID
}
...
interface ChatActions {
    ...
    setSelectedAgentData: (
        agent: AgentWithMetadata | null,
        systemPrompt?: SystemPrompt | null,
    ) => void; // Set complete agent data with prompt
    updateSessionAgent: (
        sessionId: string,
        agentId: string,
        systemPrompt?: SystemPrompt,
    ) => void; // Update session's agent and prompt
    initializeStore: () => void; // 初始化store，确保只创建一次默认会话
    setCurrentUser: (userId: number | null) => void; // 设置当前用户 ID
    clearUserData: () => void; // 清除当前用户的所有数据
}
 ...
 * Storage is isolated per user using dynamic storage key
 */
                ...
                if (session) {
                    ...
                    import("./promptStore")
                        .then(({ usePromptStore }) => {
                            ...
                            const agentData = promptStore.getAgentById(
                        ...
                        .catch((error) => {
                            console.error(
                                "Failed to load agent data during session switch:",
                                error,
                            );
                            // 回退到基本设置
                            set({
...
export const useSelectedAgentData = () =>
    useChatStore((state) => state.selectedAgentData);
export const useCurrentSystemPrompt = () =>
    useChatStore((state) => state.currentSystemPrompt);
