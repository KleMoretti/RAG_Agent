    ...
    def _build_patterns(self) -> Dict[SteelRelationType, List[str]]:
        ...
        return {
            SteelRelationType.CONTAINS: [
                r'(\w+)\s*(?:含有|包含|包括|含有|含)\s*(\w+)',
                r'(\w+)\s*(?:的|中)\s*(?:成分|组成|元素)\s*(?:包括|含有|为)\s*(\w+)',
            ],
            SteelRelationType.HAS_PROPERTY: [
                r'(\w+)\s*(?:具有|拥有|具备|表现出)\s*(\w+)',
                r'(\w+)\s*(?:的|其)\s*(?:性能|特性|性质)\s*(?:包括|为|是)\s*(\w+)',
            ],
            SteelRelationType.PRODUCED_BY: [
                r'(\w+)\s*(?:通过|采用|使用)\s*(\w+)\s*(?:生产|制造|制备)',
                r'(\w+)\s*(?:由|通过)\s*(\w+)\s*(?:工艺|方法|技术)\s*(?:生产|制造)',
            ],
            SteelRelationType.USES_EQUIPMENT: [
                r'(\w+)\s*(?:使用|采用|利用)\s*(\w+)\s*(?:设备|装置|机械)',
                r'(\w+)\s*(?:在|通过)\s*(\w+)\s*(?:上|中)\s*(?:进行|完成)',
            ],
            SteelRelationType.USED_IN: [
                r'(\w+)\s*(?:用于|应用于|适用于)\s*(\w+)',
                r'(\w+)\s*(?:在|于)\s*(\w+)\s*(?:中|方面|领域)\s*(?:使用|应用)',
            ],
            SteelRelationType.SUITABLE_FOR: [
                r'(\w+)\s*(?:适合|适用于|适用于)\s*(\w+)',
                r'(\w+)\s*(?:可以|能够)\s*(?:用于|应用于)\s*(\w+)',
            ],
            SteelRelationType.COMPLIES_WITH: [
                r'(\w+)\s*(?:符合|遵循|遵守)\s*(\w+)',
                r'(\w+)\s*(?:按照|依据|根据)\s*(\w+)\s*(?:标准|规范|要求)',
            ],
            SteelRelationType.IMPROVES: [
                r'(\w+)\s*(?:提高|改善|增强|提升)\s*(\w+)',
                r'(\w+)\s*(?:使|让|让)\s*(\w+)\s*(?:提高|改善|增强)',
            ],
            SteelRelationType.REDUCES: [
                r'(\w+)\s*(?:降低|减少|减轻)\s*(\w+)',
                r'(\w+)\s*(?:使|让|让)\s*(\w+)\s*(?:降低|减少|减轻)',
            ],
            SteelRelationType.RELATED_TO: [
                r'(\w+)\s*(?:与|和|及)\s*(\w+)\s*(?:相关|有关|联系)',
                r'(\w+)\s*(?:和|与)\s*(\w+)\s*(?:之间|的)\s*(?:关系|关联)',
            ],
            SteelRelationType.PART_OF: [
                r'(\w+)\s*(?:是|为|作为)\s*(\w+)\s*(?:的|中)\s*(?:一部分|组成部分|部分)',
                r'(\w+)\s*(?:属于|归入)\s*(\w+)',
            ],
            SteelRelationType.CAUSES: [
                r'(\w+)\s*(?:导致|引起|造成|产生)\s*(\w+)',
                r'(\w+)\s*(?:使|让|让)\s*(\w+)\s*(?:发生|出现|产生)',
            ],
        }
